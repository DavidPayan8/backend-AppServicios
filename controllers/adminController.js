const { darAltaEmpleado, getEmpleados, ordenesEmpleadoValidos, ordenesVacacionValidos, getDetalles, editarEmpleado, getVacaciones, getVacacion, actualizarVacacion, getCambiosEstado } = require("../models/adminModel");
...
const getVacacionesHandler = async (req, res) => {
	try {
		let { pagina, itemsPorPagina, filtros, ordenarPor, esAscendiente } = req.body;
		...
		if (!Number.isInteger(itemsPorPagina) || itemsPorPagina < 1) {
			res.statusMessage = "Campo 'itemsPorPagina' es obligatorio y debe ser un número natural"
			res.status(400).send();
			return;
		}
		...
		const vacaciones = await getVacaciones(req.user.id, pagina, itemsPorPagina, ordenarPor, esAscendiente, filtros);
		res.json(vacaciones);
	} catch (error) {
