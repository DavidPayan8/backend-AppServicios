...
const { mapTarifasCategorias, resourceTarifaCategoria } = require('../resources/tarifaCategoria');
const { haySolapamiento, obtenerTarifasExistentes } = require('../utils/tarifas');
...
const deleteCategoria = async (req, res) => {
    ...
    try {
        ...
        await db.TARIFAS_CATEGORIAS.destroy({ where: { id_grupo: id }, transaction: t });

        const deleted = await db.CATEGORIA_LABORAL.destroy({ where: { id }, transaction: t });
        if (!deleted) {
            await t.rollback();
        ...
        await t.commit();
        res.status(200).json({ message: 'Categoría y tarifas eliminadas con éxito' });
    } catch (error) {
        await t.rollback();
...
const createTarifa = async (req, res) => {
    try {
        const { id_grupo, horas_jornada, salario_base, fecha_inicio, fecha_fin } = req.body;

        const existentes = await obtenerTarifasExistentes(db, id_grupo, req.user.empresa);
        const solapada = haySolapamiento(existentes, new Date(fecha_inicio), fecha_fin ? new Date(fecha_fin) : null);

        if (solapada) {
            return res.status(400).json({ message: 'Ya existe una tarifa activa o solapada para esta categoría laboral.' });
        }
...
const updateTarifa = async (req, res) => {
    try {
      ...
      const existentes = await obtenerTarifasExistentes(db, id_grupo, req.user.empresa, id);
      const solapada = haySolapamiento(existentes, new Date(fecha_inicio), fecha_fin ? new Date(fecha_fin) : null);
      ...
      res.status(200).json({ message: 'Tarifa actualizada correctamente', tarifa });
    } catch (error) {
...
const deleteTarifa = async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await db.TARIFAS_CATEGORIAS.destroy({ where: { id_tarifa: id } });
        if (!deleted) return res.status(404).json({ message: 'Tarifa no encontrada' });
